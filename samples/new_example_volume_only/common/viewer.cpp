...
static void
glfwindow_mouseButton_cb(GLFWwindow *window, int button, int action, int mods) {
...
static void
glfwindow_key_cb(GLFWwindow *window, int key, int scancode, int action, int mods) {
  /* action handled by ImGui */
...
static void
glfwindow_mouseMotion_cb(GLFWwindow *window, double x, double y) {
  Viewer *gw = static_cast<Viewer *>(glfwGetWindowUserPointer(window));
  assert(gw);
...
inline owl::common::Quaternion3f
Viewer::rotation(const owl::common::linear3f &a) {
  float tr = a.vx.x + a.vy.y + a.vz.z + 1;
  float3 diag = float3(a.vx.x, a.vy.y, a.vz.z);
  if (tr > 1) {
    float s = sqrtf(tr) * 2;
    return owl::common::Quaternion3f(.25f * s, (a.vz.y - a.vy.z) / s, (a.vx.z - a.vz.x) / s, (a.vy.x - a.vx.y) / s);
  } else if (arg_min(diag) == 0) {
    float s = sqrtf(1.f + diag.x - diag.y - diag.z) * 2.f;
    return owl::common::Quaternion3f((a.vz.y - a.vy.z) / s, .25f * s, (a.vx.y - a.vy.x) / s, (a.vx.z - a.vz.x) / s);
  } else if (arg_min(diag) == 1) {
    float s = sqrtf(1.f + diag.y - diag.x - diag.z) * 2.f;
    return owl::common::Quaternion3f((a.vx.z - a.vz.x) / s, (a.vx.y - a.vy.x) / s, .25f * s, (a.vy.z - a.vz.y) / s);
  } else {
    float s = sqrtf(1.f + diag.z - diag.x - diag.y) * 2.f;
    return owl::common::Quaternion3f((a.vy.x - a.vx.y) / s, (a.vx.z - a.vz.x) / s, (a.vy.z - a.vz.y) / s, .25f * s);
  }
}

void
Viewer::mouseButtonLeft(const int2 &where, bool pressed) {
...
void
Viewer::mouseButtonRight(const int2 &where, bool pressed) {
  std::cout << "Right Click" << std::endl;
  lastMousePosition = where;
}

void
Viewer::mouseButton(GLFWwindow *window, int button, int action, int mods) {
...
inline owl::common::linear3f
Viewer::ling_rotate(const float3 &_u, const float &r) {
...
void
Viewer::rotate(const float deg_u, const float deg_v) {
  ...
  fc.frame =
      owl::common::linear3f::rotate(fc.frame.vy, rad_u) * owl::common::linear3f::rotate(fc.frame.vx, rad_v) * fc.frame;
...
float3
Viewer::arcballProject(const int2 &where) {
...
void
Viewer::mouseDragLeft(const int2 &where, const int2 &delta) {
  if (ARCBALL) {
    ...
    rotate(fraction.x * degrees_per_drag_fraction, fraction.y * degrees_per_drag_fraction);
  }
}

void
Viewer::move(const float step) {
  Camera &fc = this->camera;
...
void
Viewer::mouseDragRight(const int2 &where, const int2 &delta) {
  const float2 fraction = float2(delta) / float2(vulkanResources.configureImgui.fbSize);
...
void
Viewer::mouseMotion(const int2 &newMousePosition) {
...
void
Viewer::key(int key, const int2 &where) {
  switch (key) {
  // case 'p':
...
void
Viewer::init(GPRTProgram new_example_deviceCode) {
  initWindow();
  camera.setOrientation(lookFrom, lookAt, lookUp, cosFovy);
...
Viewer::initWindow() {
  ...
  GLFWwindow *window = glfwCreateWindow(vulkanResources.configureImgui.fbSize.x,
                                        vulkanResources.configureImgui.fbSize.y, "New Example", NULL, NULL);
  if (!window) {
    glfwTerminate();
    exit(EXIT_FAILURE);
  }
...
Viewer::run() {
  while (!glfwWindowShouldClose(handle)) {
    ...
    gprtRayGenLaunch2D(vulkanResources.context, vulkanResources.rayGen, vulkanResources.configureImgui.fbSize.x,
                       vulkanResources.configureImgui.fbSize.y);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, texFormat, vulkanResources.configureImgui.fbSize.x,
                 vulkanResources.configureImgui.fbSize.y, 0, GL_RGBA, texelType, pixels);
    ...
    glLoadIdentity();
    glOrtho(0.f, (float) vulkanResources.configureImgui.fbSize.x, (float) vulkanResources.configureImgui.fbSize.y, 0.f,
            -1.f, 1.f);
    glBegin(GL_QUADS);
    {
    ...
    // Refresh
    if (vulkanResources.configureImgui.updateVolumeSelection) {
      firstFrame = true;
      vulkanResources.configureImgui.accId = 0;

...
void
VulkanResources::createGeometry(Obj &obj) {
  // ------------------------------------------------------------------
  // triangle mesh
  // ------------------------------------------------------------------
  Geometry geometry;
  ...
  loadModel(obj, geometry.list_of_vertices, geometry.list_of_indices, geometry.list_of_colors,
            geometry.list_of_vertex_normals);
  geometry.vertexBuffer =
      gprtDeviceBufferCreate<float3>(context, geometry.list_of_vertices.size(), geometry.list_of_vertices.data());
  geometry.indexBuffer =
      gprtDeviceBufferCreate<int3>(context, geometry.list_of_indices.size(), geometry.list_of_indices.data());
  geometry.normalBuffer = gprtDeviceBufferCreate<float3>(context, geometry.list_of_vertex_normals.size(),
                                                         geometry.list_of_vertex_normals.data());
  geometry.colorBuffer =
      gprtDeviceBufferCreate<float3>(context, geometry.list_of_colors.size(), geometry.list_of_colors.data());
  ...
  gprtAccelBuild(context, geometry.trianglesBLAS, GPRT_BUILD_MODE_FAST_TRACE_NO_UPDATE);
...
VulkanResources::createVolume(Volume &volume) {
  ...
  newGeometryVolume.volumeBuffer = gprtDeviceBufferCreate<float>(context, volume_size, (float *) volume.data->data());
...
void
VulkanResources::updateGeometryMaterial(Geometry &geometry, Obj &obj) {
  loadMaterials(obj, geometry.obj_material_type, geometry.lambertian_albedo, geometry.metal_albedo, geometry.metal_fuzz,
                geometry.dielectric_ref_idx);

  geometry.materialTypeBuffer = gprtDeviceBufferCreate<int>(context, 1, &geometry.obj_material_type);
  geometry.lambertianBuffer = gprtDeviceBufferCreate<float3>(context, 1, &geometry.lambertian_albedo);
  geometry.metalAlbedoBuffer = gprtDeviceBufferCreate<float3>(context, 1, &geometry.metal_albedo);
  geometry.metalFuzzBuffer = gprtDeviceBufferCreate<float>(context, 1, &geometry.metal_fuzz);
  geometry.dielectricBuffer = gprtDeviceBufferCreate<float>(context, 1, &geometry.dielectric_ref_idx);
...
void
VulkanResources::resetVulkanGeometryResources(GPRTProgram deviceCode) {
...
VulkanResources::updateVulkanResources() {
  ...
  gprtAccelBuild(context, trashGeometry.trianglesBLAS, GPRT_BUILD_MODE_FAST_TRACE_NO_UPDATE);
...
void
VulkanResources::createVolumeAccel() {
  ...
  gprtAccelBuild(context, aabbTLAS, GPRT_BUILD_MODE_FAST_TRACE_NO_UPDATE);
}
...
void
VulkanResources::loadModel(Obj &obj, std::vector<float3> &list_of_vertices, std::vector<int3> &list_of_indices,
                           std::vector<float3> &list_of_colors, std::vector<float3> &list_of_vertex_normals) {
...
void
VulkanResources::loadMaterials(Obj &obj, int &obj_material_type, float3 &lambertians_albedo, float3 &metals_albedo,
                               float &metals_fuzz, float &dielectrics_ref_idx) {
...
void
VulkanResources::loadMaterials(Obj &obj, int list_of_indices_size, std::vector<int> &list_of_material_type,
                               std::vector<float3> &list_of_lambertians, std::vector<float3> &list_of_metals_albedo,
                               std::vector<float> &list_of_metals_fuzz, std::vector<float> &list_of_dielectrics) {
  Material obj_material = obj.material;
  std::string material_type = obj_material.type;

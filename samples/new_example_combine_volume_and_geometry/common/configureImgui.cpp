...
void
ConfigureImgui::addObjInstance(Obj &obj) {
  ...
  for (int eachObj = 0; eachObj < LIST_OF_OBJS.size(); eachObj++) {
    ...
    if (LIST_OF_OBJS[eachObj].openInstanceWindow) {
      ...
      if (ImGui::BeginCombo(" Instance",
                            LIST_OF_OBJS[eachObj].current_instance))   // The second parameter is the label previewed
                                                                       // before opening the combo.
      {
        for (int n = 0; n < LIST_OF_OBJS[eachObj].instances.size(); n++) {
          ...
          bool is_selected =
              (LIST_OF_OBJS[eachObj].current_instance ==
...
void
ConfigureImgui::updateTransform(Obj &obj) {
  inputAndSlider(obj.instances[obj.current_instance_index].translate, -100000.f, 100000.f, "Translate",
                 "Translate Input", "Translate Slider", updateObjTranslate);
  inputAndSlider(obj.instances[obj.current_instance_index].scale, 1.f, 100000.f, "Scale", "Scale Input", "Scale Slider",
                 updateObjScale);
  inputAndSlider(obj.instances[obj.current_instance_index].rotate, -10.f, 10.f, "Rotate", "Rotate Input",
                 "Rotate Slider", updateObjRotate);
}
...
void
ConfigureImgui::updateMaterialDetail(Obj &obj) {

  if (obj.current_instance_material == ALL_MATERIALS[0]) {
    inputAndSlider(obj.material.lambertian.albedo, 0.f, 1.f, "Albedo", "Lambertian Albedo Input",
                   "Lambertian Albedo Slider", updateObjMaterials);
  } else if (obj.current_instance_material == ALL_MATERIALS[1]) {
    inputAndSlider(obj.material.metal.albedo, 0.f, 5.f, "Albedo", "Metal Albedo Input", "Metal Albedo Slider",
                   updateObjMaterials);
...
void
ConfigureImgui::updateMaterial(Obj &obj) {
  obj.current_instance_material = obj.material.type;
  ImGui::Text("Loaded ");
  ImGui::SameLine();
  if (ImGui::BeginCombo(
          " Material",
          obj.current_instance_material))   // The second parameter is the label previewed before opening the combo.
  {
    for (int n = 0; n < ALL_MATERIALS.size(); n++) {
      const char *material = ALL_MATERIALS[n].c_str();
      bool is_selected = (obj.current_instance_material ==
                          material);   // You can store your selection however you want, outside or inside your objects

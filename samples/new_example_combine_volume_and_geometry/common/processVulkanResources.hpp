...
struct VulkanResources {
  ...
  void loadModel(Obj &obj, std::vector<float3> &list_of_vertices, std::vector<int3> &list_of_indices,
                 std::vector<float3> &list_of_colors, std::vector<float3> &list_of_vertex_normals);

  void loadMaterials(Obj &obj, int &obj_material_type, float3 &lambertian_albedo, float3 &metal_albedo,
                     float &metal_fuzz, float &dielectric_ref_idx);

  void loadMaterials(Obj &obj, int list_of_indices_size, std::vector<int> &list_of_material_type,

...
VulkanResources::createVolumeAndGeometryAccel() {
  ...
  if (listOfObjAndVolumesBLAS.size() > 0) {
    ...
    transformObjAndVolumeBuffer =
        gprtDeviceBufferCreate<float4x4>(context, transformsObjAndVolume.size(), transformsObjAndVolume.data());
  ...
  gprtAccelBuild(context, objAndVolumeTLAS, GPRT_BUILD_MODE_FAST_TRACE_NO_UPDATE);
}
